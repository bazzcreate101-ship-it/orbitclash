...
- Version code: `104`
- APK filename: `orbit-clash-matchsim100-v104.apk`
- On-screen marker: `BUILD 104 | stable signed multi-package | v1.0.4`
...
The preset points to `builds/orbit-clash-matchsim100-v104.apk`.

CLI after Android tooling is configured:
`godot --headless --path . --export-debug "Android Debug" builds/orbit-clash-matchsim100-v104.apk`
...
3. Download `orbit-clash-matchsim100-v104-stable-signed-replacements`.
4. Extract the ZIP.
5. Install `orbit-clash-matchsim100-v104.apk` on an Android device.
...
If the screen says `ORBIT CLASH LOADING ARENA`, that is an old package/build. The current app must show the permanent marker `BUILD 104 | stable signed multi-package | v1.0.4`.

See [INSTALL_ANDROID.md](INSTALL_ANDROID.md) for clean-install steps and replacement APKs for old package names.
