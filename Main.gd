...
var roster := [
    {"id":"outlaw", "name":"OUTLAW", "color":Color("d79b5f"), "weapon":Color("ffe0b8"), "skill":"High Noon", "desc":"Heat rises over time and on contact. Full heat detonates and boosts the spin tempo."},
    {"id":"stasis", "name":"STASIS", "color":Color("77c7f2"), "weapon":Color("def7ff"), "skill":"Deep Freeze", "desc":"Hits chill the enemy. The freeze meter can lock movement and reset momentum."},
    {"id":"retaliator", "name":"RETALIATOR", "color":Color("7d8cff"), "weapon":Color("d9ddff"), "skill":"Karma", "desc":"Damage taken powers up counter damage. The lower the HP, the meaner the comeback."},
    {"id":"rogue", "name":"ROGUE", "color":Color("b47ef5"), "weapon":Color("edd9ff"), "skill":"Rising Crit", "desc":"Crit chance climbs after every strike. A full surge triggers a dash burst."},
    {"id":"swordsaint", "name":"SWORD SAINT", "color":Color("6fd0b5"), "weapon":Color("d8fff1"), "skill":"Perfect Cut", "desc":"Clean hits build focus. Full focus unleashes a sharp power slash."},
    {"id":"judge", "name":"JUDGE", "color":Color("d7a44a"), "weapon":Color("fff0bf"), "skill":"Verdict", "desc":"Karma rises on every exchange. Full karma fires a judgment shot from the body."},
    {"id":"monkeyking", "name":"MONKEY KING", "color":Color("e7c34f"), "weapon":Color("fff1a9"), "skill":"Echo Staff", "desc":"Hits grow the staff. Every few hits spawn another orbiting echo weapon."},
    {"id":"dragoon", "name":"DRAGOON", "color":Color("6c5ce7"), "weapon":Color("d8d0ff"), "skill":"Dragon Arsenal", "desc":"Hard hits increase damage. Full meter summons a radial lance volley."},
    {"id":"tyrant", "name":"TYRANT", "color":Color("c18a2f"), "weapon":Color("ffe2a0"), "skill":"Imperial Gates", "desc":"Builds gates over time. Gates periodically fire blades across the arena."},
    {"id":"magia", "name":"MAGIA", "color":Color("ef7fb0"), "weapon":Color("ffd5ea"), "skill":"Spell Bloom", "desc":"Magic builds into shockwaves and spell blooms that punish tight fights."}
]
var selected_left := 0
...
func _make_fighter(data: Dictionary, pos: Vector2, vel: Vector2, side: int) -> Dictionary:
    ...
    match data.id:
        ...
        "swordsaint":
            f.weapon_len = 82.0; f.damage = 4.8; f.base_spin = 3.15 if side == 0 else -3.15; f.spin = f.base_spin
        "retaliator":
            f.weapon_len = 88.0; f.damage = 2.8; f.base_spin = 2.65 if side == 0 else -2.65; f.spin = f.base_spin
        "judge":
            f.weapon_len = 76.0; f.damage = 4.6; f.base_spin = 2.85 if side == 0 else -2.85; f.spin = f.base_spin
        "dragoon":
        ...
        "magia":
            f.weapon_len = 78.0; f.damage = 4.3; f.base_spin = 3.1 if side == 0 else -3.1; f.spin = f.base_spin
    return f
...
func _update_fighter(f: Dictionary, delta: float, index: int) -> void:
    ...
    match f.id:
        ...
        "swordsaint":
            f.focus = minf(1.0, f.focus + delta * 0.028)
            f.spin = f.base_spin * (1.0 + f.focus * 0.42)
            if f.focus >= 1.0:
                f.focus = 0.0
                f.ability_flash = 1.0
                f.attack_scale = minf(2.35, f.attack_scale + 0.10)
                _spawn_ring(f.pos, f.body_color, 18); _float_text(f.pos + Vector2(0,-60), "PERFECT", f.body_color); _play("ability")
        "retaliator":
            f.meter = minf(1.0, f.meter + delta * 0.018)
            if f.meter >= 1.0: _deprivation(f)
            f.damage = 2.8 + f.retaliation
        "judge":
            f.karma = minf(1.0, f.karma + delta * 0.026)
            if f.karma >= 1.0:
                f.karma = 0.0; f.ability_flash = 1.0
                var target_side := 1 - f.side
                if target_side >= 0 and target_side < fighters.size():
                    var target: Dictionary = fighters[target_side]
                    _spawn_projectile(f.side, f.pos + Vector2.RIGHT.rotated(f.angle) * 30.0, (target.pos - f.pos).normalized() * 430.0, 8.0, 1.4, f.body_color, "verdict")
                _float_text(f.pos + Vector2(0,-58), "VERDICT", f.body_color); _spawn_ring(f.pos, f.body_color, 16); _play("ability")
        "dragoon":
        ...
        "magia":
            f.magic = minf(1.0, f.magic + delta * 0.03)
            f.attack_scale = 1.0 + f.magic * 0.56
            f.spin = f.base_spin * (1.0 + f.magic * 0.38)
            if f.magic >= 1.0:
                f.magic = 0.0
                f.ability_flash = 1.0
                zones.append({"type":"heart", "owner":f.side, "pos":f.pos, "life":0.95, "max":0.95, "tick":0.0, "radius":32.0, "color":f.body_color})
                _spawn_shockwave(f.pos, f.body_color, 0.62); _float_text(f.pos + Vector2(0,-58), "SPELL BURN", f.body_color); _play("ability")

    f.pos += f.vel * delta * slow_mult
    _bounce_arena(f)
    var max_speed := 400.0
    if f.id == "rogue": max_speed = 460.0
    ...
    for seg in _weapon_segments(attacker):
        if _distance_point_segment(target.pos, seg[0], seg[1]) <= RADIUS + float(seg[2]) * 0.6:
            ...
            match attacker.id:
                ...
                "swordsaint":
                    attacker.focus = minf(1.0, attacker.focus + 0.16)
                    attacker.attack_scale = minf(2.2, attacker.attack_scale + 0.03)
                    if attacker.focus >= 1.0:
                        dmg *= 1.95; attacker.focus = 0.0; attacker.ability_flash = 1.0
                        _float_text(attacker.pos + Vector2(0,-52), "PERFECT CUT", attacker.body_color); _spawn_ring(attacker.pos, attacker.body_color, 16); _play("ability")
                "retaliator":
                    attacker.meter = minf(1.0, attacker.meter + 0.08)
                "judge":
                    attacker.karma = minf(1.0, attacker.karma + 0.18)
                    if attacker.karma >= 1.0:
                        attacker.karma = 0.0
                        _spawn_projectile(attacker.side, attacker.pos + Vector2.RIGHT.rotated(attacker.angle) * 30.0, Vector2.RIGHT.rotated(attacker.angle) * 390.0, 7.2, 1.6, attacker.body_color, "verdict")
                        _play("ability")
                "magia":
...
func _finish_battle(winner: Dictionary) -> void:
    ...
    if speed_button != null:
        speed_button.visible = false
    _spawn_ring(winner.pos, winner.body_color, 32)
    _spawn_shockwave(winner.pos, winner.body_color, 1.0)
    _play("win")
...
func _make_ui() -> void:
    ...
    if mobile_build:
        ...
        mobile_left_prev_button = Button.new(); mobile_left_prev_button.text = "< LEFT"; mobile_left_prev_button.position = Vector2(24,944); mobile_left_prev_button.size = Vector2(140,52); mobile_left_prev_button.pressed.connect(func(): _cycle_selection(-1, true)); add_child(mobile_left_prev_button)
        mobile_left_next_button = Button.new(); mobile_left_next_button.text = "LEFT >"; mobile_left_next_button.position = Vector2(172,944); mobile_left_next_button.size = Vector2(140,52); mobile_left_next_button.pressed.connect(func(): _cycle_selection(1, true)); add_child(mobile_left_next_button)
        mobile_right_prev_button = Button.new(); mobile_right_prev_button.text = "< RIGHT"; mobile_right_prev_button.position = Vector2(24,1004); mobile_right_prev_button.size = Vector2(140,52); mobile_right_prev_button.pressed.connect(func(): _cycle_selection(-1, false)); add_child(mobile_right_prev_button)
        mobile_right_next_button = Button.new(); mobile_right_next_button.text = "RIGHT >"; mobile_right_next_button.position = Vector2(172,1004); mobile_right_next_button.size = Vector2(140,52); mobile_right_next_button.pressed.connect(func(): _cycle_selection(1, false)); add_child(mobile_right_next_button)
        mobile_random_button = Button.new(); mobile_random_button.text = "RANDOM"; mobile_random_button.position = Vector2(376,944); mobile_random_button.size = Vector2(140,52); mobile_random_button.pressed.connect(_randomize_matchup); add_child(mobile_random_button)
        mobile_start_button = Button.new(); mobile_start_button.text = "RESTART"; mobile_start_button.position = Vector2(524,944); mobile_start_button.size = Vector2(140,52); mobile_start_button.pressed.connect(_start_battle); add_child(mobile_start_button)
        pause_button = Button.new(); pause_button.text = "II"; pause_button.position = Vector2(530,118); pause_button.size = Vector2(54,48); pause_button.visible = true; pause_button.pressed.connect(_toggle_pause); add_child(pause_button)
        speed_button = Button.new(); speed_button.text = "1x"; speed_button.position = Vector2(590,118); speed_button.size = Vector2(54,48); speed_button.visible = true; speed_button.pressed.connect(_cycle_speed); add_child(speed_button)
        sound_button = Button.new(); sound_button.text = "SFX"; sound_button.position = Vector2(650,118); sound_button.size = Vector2(54,48); sound_button.visible = true; sound_button.pressed.connect(_toggle_sound); add_child(sound_button)
...
func _randomize_matchup() -> void:
    ...
    if mobile_build and state != "menu":
        _start_battle()
...
func _cycle_speed() -> void:
    ...
    if speed_button != null:
        speed_button.text = "%gx" % sim_speed
...
func _fighter_stat_line(f: Dictionary) -> String:
    match f.id:
        "monkeyking": return "STAFF %.2fX   ECHO %d" % [f.size_mult,f.clones]
        "rogue": return "CRIT %d%%   SURGE %d%%" % [int(f.crit*100.0),int(f.dash_meter*100.0)]
        "swordsaint": return "FOCUS %d%%   SCALE %.2fX" % [int(f.focus*100.0),f.attack_scale]
        "retaliator": return "DMG %.2f   KARMA %d%%" % [f.damage,int(f.meter*100.0)]
        "judge": return "KARMA %d%%   VERDICT" % [int(f.karma*100.0)]
        "dragoon": return "DMG %.2f   ARSENAL %d" % [f.damage,f.arsenal]
        "tyrant": return "GATES %d   NEXT %d%%" % [f.gate_count,int(clampf(f.gate_timer/2.15,0,1)*100)]
        "outlaw": return "HEAT %d%%   SPIN %.2f" % [int(f.heat*100.0),absf(f.spin)]
        "stasis": return "FREEZE %d%%   SPIN %.2f" % [int(f.meter*100.0),absf(f.spin)]
        "magia": return "MANA %d%%   SPELL %.2fX" % [int(f.magic*100.0),f.attack_scale]
    return ""
func _ability_ratio(f: Dictionary) -> float:
    match f.id:
        "monkeyking": return float(f.hit_count % 5)/5.0
        "rogue": return f.dash_meter
        "swordsaint": return f.focus
        "retaliator","dragoon","stasis": return f.meter
        "tyrant": return clampf(f.gate_timer/2.15,0.0,1.0)
        "outlaw": return f.heat
        "judge": return f.karma
        "magia": return f.magic
    return 0.0
...
func _draw() -> void:
    ...
    draw_rect(Rect2(0,0,W,H),Color("f4f2ef"),true); _draw_background_grid()
    if state != "menu":
...
func _draw_background_grid() -> void:
    for y in range(0,14):
        var yy := 120.0+y*82.0; draw_line(Vector2(0,yy),Vector2(W,yy),Color(0.45,0.42,0.38,0.05),1.0)
    for x in range(0,9):
        var xx := x*90.0; draw_line(Vector2(xx,110),Vector2(xx,H),Color(0.45,0.42,0.38,0.04),1.0)
...
func _draw_arena(off:Vector2)->void:
    var ar:=Rect2(ARENA.position+off,ARENA.size);draw_rect(ar,Color("f7f7f6"),true)
    for y in range(0,9):
        var yy:=ar.position.y+46.0+y*91.0;draw_line(Vector2(ar.position.x+8,yy),Vector2(ar.end.x-8,yy),Color(0.4,0.35,0.3,0.08),1.0)
    for x in range(0,7):
        var xx:=ar.position.x+55.0+x*92.0;draw_line(Vector2(xx,ar.position.y+8),Vector2(xx,ar.end.y-8),Color(0.4,0.35,0.3,0.05),1.0)
    draw_rect(ar,Color("27221c"),false,4.0);draw_rect(Rect2(ar.position+Vector2(8,8),ar.size-Vector2(16,16)),Color(0.2,0.17,0.15,0.03),false,1.0)
